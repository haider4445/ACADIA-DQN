...
class Trainer():
    ...
    def apply_gradient_attack(self,last_states, params, next_actions):
        ...
        if perturbationType == "rfgsm" or perturbationType == "RFGSM" or perturbationType == "Rfgsmt":
            ...
            rfgsmIns = APMRFGSM(model = net, targeted = targeted, steps = steps, eps = eps, alpha = alpha, decay = decay, decay2 = decay2)      
        elif perturbationType == "airfgm" or perturbationType == "AIRFGM":
            rfgsmIns = AIRFGM(model = net, targeted = targeted, steps = steps, eps = eps, alpha = alpha, decay = decay, decay2 = decay2)           
    ...
    def run_test_trajectories(self, params, max_len, should_tqdm=False):
        ...
        for t in iterator:
            ...
            if params['targeted'] == 1:
                max = 2
                min = -2
                true_actions = (max-min)*(torch.rand((1,8))) + min
